...
const sortedCertificates = [...certificates].sort((a, b) => parseDateValue(b.date) - parseDateValue(a.date));
  ...
  return (
    <div className="max-w-6xl mx-auto px-4  pt-32 pb-14 md:pt-44 md:pb-16">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8 mb-10">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] mb-4" style={{ color: '#A1A1AA', fontFamily: "'Space Mono', monospace" }}>
            Credentials &amp; Recognition
          </p>
          <h2 className="text-4xl md:text-6xl leading-[0.95]" style={{ color: '#FFFFFF', fontFamily: "'Playfair Display', serif", fontWeight: 500 }}>
            Achievements
          </h2>
        </div>
        <p className="max-w-sm text-sm md:text-base leading-relaxed md:text-right" style={{ color: '#A1A1AA' }}>
          Competitions, certifications, and research presentations from across my work in AI and software development.
        </p>
      </div>

      <div className="flex flex-wrap items-center justify-between gap-6 pt-6" style={{ borderTop: '1px solid #666666' }}>
          ...
          {FILTERS.map(f => {
            ...
            return (
              <button
                ...
                className="text-xs uppercase tracking-wider pb-2 px-1 transition-colors"
              >
                {f} <span style={{ color: '#666666' }}>({count})</span>
              </button>
            );
          })}
        </div>
        <p className="text-[11px] tracking-wide" style={{ color: '#666666', fontFamily: "'Space Mono', monospace" }}>
  ...
  return (
    ...
    >
      <div className="hidden md:block pt-1">
        <span className="text-[11px] tracking-wide" style={{ color: '#A1A1AA', fontFamily: "'Space Mono', monospace" }}>{cert.date}</span>
      </div>

      <div className="relative flex justify-center">
        <div className="absolute top-0 bottom-0 w-px" style={{ background: '#666666' }} />
        <span className="relative mt-1.5 w-2.5 h-2.5 rounded-full" style={{ background: '#111111', border: `2px solid ${cert.accent}` }} />
      </div>
      ...
      <div>
        ...
        <span style={{ color: cert.type === 'Professional' ? cert.accent : '#A1A1AA', fontFamily: "'Space Mono', monospace" }} className="text-[10px] uppercase tracking-[0.15em]">
          {cert.type === 'Professional' ? 'Winner' : 'Participant'}
        </span>

        <h3 className="text-lg md:text-xl leading-snug mt-2 mb-1.5" style={{ color: '#FFFFFF', fontFamily: "'Playfair Display', serif", fontWeight: 500 }}>
          {cert.title}
        </h3>
        <p className="text-sm mb-3" style={{ color: '#A1A1AA' }}>{cert.issuer}</p>
        <p className="text-sm leading-relaxed max-w-xl mb-4" style={{ color: '#A1A1AA' }}>{cert.description}</p>

        <button
          onClick={() => onSelect(cert)}
          style={{ color: cert.accent, fontFamily: "'Space Mono', monospace" }}
          className="inline-flex items-center gap-1.5 text-xs uppercase tracking-wide opacity-80 group-hover:opacity-100 transition-opacity"
        >
          View certificate
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        style={{ background: '#111111', border: '1px solid #666666', borderRadius: '8px' }}
      ...
      >
        <div className="flex flex-col md:flex-row">
          <div className="md:w-1/2 flex items-center justify-center p-6 min-h-56" style={{ background: '#000000' }}>
            <img src={cert.image} alt={cert.title} className="max-h-72 w-full object-contain rounded-md" />
          </div>
          <div className="md:w-1/2 p-7 flex flex-col justify-between" style={{ borderTop: '1px solid #666666' }}>
            <div>
              <span style={{ color: cert.type === 'Professional' ? cert.accent : '#A1A1AA', fontFamily: "'Space Mono', monospace" }} className="text-[10px] uppercase tracking-[0.15em] mb-3 inline-block">
                {cert.type === 'Professional' ? 'Winner' : 'Participant'}
              </span>
              <h3 className="text-xl leading-tight mb-2" style={{ color: '#FFFFFF', fontFamily: "'Playfair Display', serif", fontWeight: 500 }}>
                {cert.title}
              </h3>
              <p className="text-sm mb-4" style={{ color: '#A1A1AA' }}>{cert.issuer} · {cert.date}</p>
              <div className="h-px mb-4" style={{ background: '#666666' }} />
              <p className="text-sm leading-relaxed" style={{ color: '#D4D4D8' }}>{cert.description}</p>
            </div>
            <div className="mt-7 flex items-center gap-3 flex-wrap">
              <a
                href={cert.linkedIn} target="_blank" rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium transition-colors"
                style={{ background: cert.accent, color: '#111111', borderRadius: '8px' }}
              >
              ...
              <button
                onClick={onClose}
                style={{ border: '1px solid #666666', color: '#A1A1AA', borderRadius: '8px' }}
                className="px-4 py-2 text-sm hover:text-white transition-colors"
              >
                Close
              </button>
            </div>
...
export default function Achievements() {
  ...
  const visible = useMemo(() => sortedCertificates.filter(c => filterMatches(filter, c)), [filter]);

  return (
    <section className="w-full relative" style={{ background: '', color: '#FFFFFF' }}>
